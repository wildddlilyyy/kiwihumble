    ...
    <main class="min-h-screen bg-slate-100">
        ...
        <section class="mx-auto grid max-w-6xl gap-6 px-6 py-8 lg:grid-cols-[280px_1fr]">
            <aside class="rounded-xl bg-white p-5 shadow-sm ring-1 ring-black/5">
                ...
                <p class="mt-4 text-center text-sm font-semibold text-slate-600">Manage public trip details and fixed member records.</p>
            </aside>

            <div>
            <form method="POST" action="{{ route('backend.settings.update') }}" class="mb-6 h-full rounded-xl bg-white p-6 shadow-sm ring-1 ring-black/5">
                @csrf
                <div class="flex flex-col gap-2 sm:flex-row sm:items-end sm:justify-between">
                    ...
                    <button class="rounded-lg bg-kiwi-blue px-4 py-2 text-sm font-black text-white hover:bg-kiwi-ink" type="submit">Save Settings</button>
                </div>
                @if (session('status'))
                    <div class="mt-5 rounded-lg bg-emerald-50 p-3 text-sm font-bold text-emerald-700">{{ session('status') }}</div>
                @endif
                @if ($errors->any())
                ...
                <div class="mt-6 grid gap-5">
                    <label class="block">
                        ...
                        <input class="mt-2 w-full rounded-lg border-slate-300 focus:border-kiwi-blue focus:ring-kiwi-blue" type="text" name="trip_title" value="{{ old('trip_title', $settings->firstWhere('key', 'trip_title')?->value) }}" required>
                    </label>
                    <div class="grid gap-5 sm:grid-cols-2">
                        <label class="block">
                            <span class="text-sm font-bold text-slate-700">Trip Date</span>
                            <input class="mt-2 w-full rounded-lg border-slate-300 focus:border-kiwi-blue focus:ring-kiwi-blue" type="date" name="trip_date" value="{{ old('trip_date', $settings->firstWhere('key', 'trip_date')?->value) }}" required>
                        </label>
                        <label class="block">
                            <span class="text-sm font-bold text-slate-700">Timezone</span>
                            <input class="mt-2 w-full rounded-lg border-slate-300 focus:border-kiwi-blue focus:ring-kiwi-blue" type="text" name="timezone" value="{{ old('timezone', $settings->firstWhere('key', 'timezone')?->value) }}" required>
                        </label>
                    </div>
                </div>
            </form>
            ...
            <div class="overflow-x-auto rounded-xl bg-white shadow-sm ring-1 ring-black/5 lg:col-span-2">
